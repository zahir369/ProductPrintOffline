...
namespace MKSS.Model
{
    ...
    public class SensorGroupData
    {
        ...
        public static double CalcDataValueByLoad(SensorGroupData group, PosEnum position) {
            ...
            switch (ShowMode)
            {
                ...
                case ShowModeEnum.NongDu:
                    return SensorGroupDataStander.CalcDataValueND(group, position);//默认AD
        ...
        /// </summary>
        [SugarColumn(IsIgnore = true)]
        public static ShowModeEnum ShowMode { get; set; } = ShowModeEnum.DuanDianYa;
        [SugarColumn(IsIgnore = true)]
        public static string ShowModeDesc { get { return ShowModeString(ShowMode); } } 
        [SugarColumn(IsIgnore = true)]
        public static ShowModeEnum ShowModeDefault { get; set; } = ShowModeEnum.DuanDianYa;
        ...
        public static string ShowDesc(SensorGroupData g, PosEnum p)
        {
            ...
            return String.Format("{3}：{0}；{1}；{2}；T90时间：{4}秒；T10时间：{5}秒；", 
                ShowValue(ShowModeEnum.NongDu,g,p),
                ShowValue(ShowModeEnum.DuanDianYa, g, p), 
                ShowValue(ShowModeEnum.AD, g, p),
                p,
                sta1.T90.ToString("F2"),
                sta1.T10.ToString("F2")
            );
        }
        ...
        public static string ShowValue(ShowModeEnum e, SensorGroupData g,PosEnum p)
        {
            switch (e)
            {
                ...
                case ShowModeEnum.NongDu:
                    return string.Format("O₂浓度：{0}%", CalcDataValueByLoad(g, p, e));
                case ShowModeEnum.AD:
    ...
    public class SensorGroupDataStander
    {
        ...
        public const double ZeroStd = 3730;
        public const double OxygenStdPulse = 0.00571D;//139.5;//0.00571
        ...
        public static double CalcDataValueND(SensorGroupData real, PosEnum position)
        {
            ...
            if (STD!=null && STD.F_DataValue(position).F_DataValue <3700)
            {
                ...
                return double.Parse((OxygenStd * (ZeroStd - d) / (ZeroStd - d_std)).ToString("f1"));
            }
            return 0;
        }
        ...
        public static double CalcDataValueDYDY(SensorGroupData real, PosEnum position)
        {
            ...
            return double.Parse(v.ToString("f1"));
        }
