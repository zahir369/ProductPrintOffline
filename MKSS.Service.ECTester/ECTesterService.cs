...
namespace MKSS.Service.ECTester
{
	...
	public class ECTesterService
	{
		...
		///  工业传感器还是水性传感器
		/// </summary>
		public static bool IndustryMode { get { return ECSensotMode == ECSensotMode.IndustryMode; } }
		/// <summary>
		///  水性\酸性两电极\工业
		/// </summary>
		public static ECSensotMode ECSensotMode = ECTester.ECSensotMode.AqueousMode; 
		...
		public void StartTask(Batch _bat, List<Sensor> sens, ProductConfig product, string com, List<int> addr_list)
		{
			...
			{
				for (int s = 1; s <= ECTesterService.RegionSize; s++)
				{
			...
			ECTesterSaver.Batch = this.CurrentBatch;
				...
				try
				{
					...
					while (true)
					{
						try
						{
                            ...
                            //数据取舍，120秒内 不舍弃数据，120秒后 时间间隔逐渐变大，减少绘制存储负担
                            if (SpanTotal.TotalSeconds > 180)
                            {
								sleep = (int)((SpanTotal.TotalSeconds / 50) * 1000);
                            } 
				...
				try
				{
					...
					for (int r = 0; r < region.Length; r++)
					{
						...
						try
						{
							...
							if (!Debug)
							{
								ret = _master.ReadHoldingRegisters((byte)address, (ushort)1, (ushort)ECTesterService.RegionSize);
							}
							else {
								double t = this.SpanTotal.TotalSeconds;
								ret = GetRandNums(0, 650, ECTesterService.RegionSize).ToArray();
								Thread.Sleep(200);
							}
						...
						for (int i = 0; i < ECTesterService.RegionSize; i++)
						{
							if (Cancel || Pause) return;
					...
					try
					{
						...
						if (any_read_sucess)
						{
							...
							if (Cancel || Pause) return;
							ECTesterSaver.EnqueueTask(sb.ToString());
		...
		public bool TestConn(string COM, List<int> addr_list)
		{
			...
			try
			{
				...
				for (int r = 0; r < ECTesterService.Regions.Length; r++)
				{
					string boardNo = ECTesterService.Regions[r];
					int address = addr_list[r];
					ushort[] ret = null;
					try
					{
		...
		public void InitDataByDb(Batch _bat) {
			List<Sensor> sens = ECTesterSaver.SensorServices.BaseDal.Db.Queryable<Sensor>().Where(w=>w.F_BatchId== _bat.F_BatchId).ToList();
			try
			{
				...
				DataTable table = ECTesterSaver.SensorDataServices.QueryTable("select * from pd_sensorgroupdata_" + _bat.F_BatchId + " order by F_AddTime").Result;
		...
		public static Batch OfBatch(long F_BatchId)
		{
			...
			try
			{
				return ECTesterSaver.SensorServices.BaseDal.Db.Queryable<Batch>().Where(w => w.F_BatchId == F_BatchId).First();
			}
			catch (Exception ex)
			{
		...
		public static List<SensorGroupData> QueryData(long F_BatchId, double span = -1, int floorNO = -1)
		{
			...
			try
			{
				...
				return ECTesterSaver.SensorServices.BaseDal.Db.SqlQueryable<SensorGroupData>(
