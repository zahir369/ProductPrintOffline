...
{
    [LogTagClass(Title = "老化数据")]
	public class UILaoHuaGuanChaData
    {

		static UILaoHuaGuanChaData _instance = null;
		public static UILaoHuaGuanChaData Instance
        {
			get {
				if (_instance == null) {
					_instance = new UILaoHuaGuanChaData();
		...
		public bool Exists(string scrwd, long NewSerialNo, long OldSerialNo)
		{
			...
			DataTable table = BoardServices.BaseDal.Db.Ado.GetDataTableAsync(strSql).Result;
		...
		{
			var rezz = BoardServices.BaseDal.Db.Ado.ExecuteCommandAsync(string.Format("insert into pd_sensor_scrwd (F_SCRWD_OrderNumber,F_SerialNO) VALUES ('{0}', {1});", scrwd, NewSerialNo)).Result;
		}
		...
		/// <param name="count"></param>
		/// <returns></returns>
		public int ExecuteCommandAsync(string sql )
		{
			return BoardServices.BaseDal.Db.Ado.ExecuteCommandAsync(sql).Result;
		}
		...
		Dictionary<long, DateTime> GetSensorCacheDt = new Dictionary<long, DateTime>();
		Dictionary<long, List<Sensor>> GetSensorCache = new Dictionary<long, List<Sensor>>();
		public List<Sensor> GetSensor(Batch batch) {

			TimeSpan ts = new TimeSpan(0, 5, 0);
			if (!GetSensorCache.ContainsKey(batch.F_BatchId))
			{
				GetSensorCacheDt.Add(batch.F_BatchId, DateTime.Now);
				GetSensorCache.Add(batch.F_BatchId, this.SensorServices.QuerySql("select * from pd_sensor where F_BatchId=" + batch.F_BatchId).Result);
			}
			else {
				if (DateTime.Now - GetSensorCacheDt[batch.F_BatchId] > ts) {
					GetSensorCacheDt[batch.F_BatchId] = DateTime.Now;
					GetSensorCache[batch.F_BatchId] = this.SensorServices.QuerySql("select * from pd_sensor where F_BatchId=" + batch.F_BatchId).Result;
				}
			}

			return GetSensorCache[batch.F_BatchId];
		}
			...
			ref BoardCase _BoardCase,ref Board _Board )
		{
			...
			_Board = listBoard.FirstOrDefault(w => w.AddrList.Contains((byte)DataSrc.Address));
		...
		public Batch OfBatch(PageBoardItem DataSrc,
			ref BoardCase _BoardCase, ref Board _Board,ref List<Sensor> senList)
		{
			if (DataSrc.Address <= 0 && DataSrc.CaseNo <= 0) return null;
			_BoardCase = this.DBListBoardCase.FirstOrDefault(w => w.F_BoardCaseAddress == DataSrc.CaseNo);
			if (_BoardCase == null) return null;
		...
		public void BtnFinishBoardExe(PageBoardItem DataSrc)
		{
			try
			{
				...
				if (_BoardCase != null && DBBoardCaseDictionary.ContainsKey(_BoardCase))
				{
					...
					{
						ULogger.Info((string.Format("找不到老化柜{0}地址{1}", DataSrc.CaseNo, DataSrc.Address)));
					}
				}
				else
				{
					ULogger.Info((string.Format("找不到老化柜{0}", DataSrc.CaseNo)));
				}
			}
			catch (Exception ex)
			{
				ULogger.Info((string.Format("结束老化出错：{0}", ex.Message)));
			}
		...
		public void RestartBatch(Batch _Batch)
		{
			...
			foreach (var rr in rel)
			{
				...
				{
					ULogger.Error(string.Format("重启老化出错：{0}", ex.Message));
					ULogger.Error(ex);
				}
			}
		...
		public void DeleteBatch(Batch batch)
		{
			...
			{
				ULogger.Info((string.Format("删除老化任务出错 {0} ......", ex.Message)));
				throw ex;
			}
