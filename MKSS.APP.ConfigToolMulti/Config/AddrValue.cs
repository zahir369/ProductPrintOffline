...
namespace MKSS.APP.ConfigToolMulti
{
    ...
    public class AddrValue : AddrConfig {
		...
		public AddrValue CreateCopy() {
			AddrValue ret = new AddrValue();
            foreach (PropertyInfo item in ret.GetType().GetProperties())
            {
				if (item.CanRead && item.CanWrite) item.SetValue(ret, item.GetValue(this));
			}
			return ret;
		}
