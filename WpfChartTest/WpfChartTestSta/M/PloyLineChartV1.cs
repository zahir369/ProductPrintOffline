...
namespace MKSS.APP.ECTester
{
    ...
    public class PloyLineChart
    {
        ...
        public const float YTopMargin = 35;
        private float YLabelLen = 8;
        private const int YAxisCount = 5;
        private const int XAxisCount = 12;
        ...
        public void AppendData(float x, List<float> yList)
        {
            ...
            for (int count = 0; count < AllDatas.Count; count++)
            {
                ...
                if (
                    ...
                    ||
                    x_end > X0 + XWidth || y_end > Y0 + YHeight
                    )
                    continue;
        ...
        void AppendLineSegment(PointF p1, PointF p2, Graphics dc, int count, bool firstPoi)
        {
            ...
            {
                PointF[] ps = GetIntersectionPoint(p1, p2, rect, StartWithIn, EndWithIn);
                if (ps == null) return;
                p1 = ps[0];
                p2 = ps[1];
            }
            ...
            if (firstPoi) dc.FillEllipse(AllDatas[count].Line1Color, p1.X - size, p1.Y - size, size * 2, size * 2);//画填充椭圆的方法，x坐标、y坐标、宽、高，如果是100，则半径为50
            dc.FillEllipse(AllDatas[count].Line1Color, p2.X - size, p2.Y - size, size * 2, size * 2);//画填充椭圆的方法，x坐标、y坐标、宽、高，如果是100，则半径为50
        ...
        /// <returns></returns>
        public static PointF[] GetIntersectionPoint(PointF poiStart, PointF poiEnd, RectangleF rect, bool StartWithIn, bool EndWithIn)
        {
            float x0 = rect.X;
