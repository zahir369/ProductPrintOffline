...
namespace DeviceDataMonitorWPF.UIBiaoDing.Util
{
    ...
    public class ModBusBoard
	{
		...
		{
			this.Address = new List<byte>();
			this.ReadSpeed = 200;
			this.Qualified = new DataQualified();
			this.DataAging = new UIBiaodingJiuJing.Util.DataAging(boardcase_no, tunnel_no);
		...
		public UIBiaodingJiuJing.Util.DataAging DataAging
		{
			get;
			set;
		}
		...
		private bool WriteSignal(DeviceDataMonitorWPF.UIBiaoDing.Signal.ISignal signal)
		{
			XFF_StopRead StopReadSignal = new XFF_StopRead();
			...
			while (this.IsRead)
			{
				...
				while (root.i < this.Address.Count)
				{
					...
					if (!error_data) {
						try
						{
							pm = new DataAgingSensorGroupData(this.DataAging, this.Address[line.Root.i]);
