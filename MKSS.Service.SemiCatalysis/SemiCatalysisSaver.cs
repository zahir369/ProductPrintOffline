...
{
    public class SemiCatalysisSaver
	{
		// 任务队列
		...
		static SensorDataServices _SensorDataServices = new SensorDataServices();
		...
		public static string DbNameof(Batch _bat) {
			...
			return string.Format(string.Format(AppDomain.CurrentDomain.BaseDirectory + @"data\MKSS.APP.SemiCatalysis.{0}.sqlite", _bat.F_BatchId));
		}
		...
		static void Work()
		{
			while (true)
			{
				...
				lock (_locker)
				{
					while (works.Count <= 16 && _tasks.Count > 0)
					{
		...
		static void SaveData(List<string> data)
		{
            try
            {
				...
				var xxx = _SensorServices.BaseDal.ExecuteCommand(s).Result;
				if (xxx == 0) {
					ULogger.Log(string.Format("插入数据出错 {0}", "Result==0"));
				}
			}
            catch (Exception ex)
            {
