...
namespace MKSS.APP.LaoHuaService.MQTT
{
    ...
    public class LaoHuaMqttClientProducer : LaoHuaMqttClient {
        ...
        public static LaoHuaMqttClientProducer Instance
        {
            get
            {
                if (_Instance == null)
                {
                    ...
                    WindwsService.DeviceMonitor.Program.LaohuaService.OnNewData += _Instance.LaohuaService_OnNewData;
                }
                return _Instance;
            }
        }
        ...
        void LaohuaService_OnNewData(Model.Batch _Batch, List<Model.SensorData> datas_history)
        {
        ...
        private Task Instance_OnGetMQData(MqttMessage mqMessage)
        {
            Task t = new Task(()=>{
                ...
                if (mqMessage.TopicName.StartsWith("LaoHua/QueryRealtimeData")) {
                    ...
                    if (int.TryParse(batch_id, out batch_id_int))
                    {
                        Dictionary<long, Dictionary<string, MKSS.Model.SensorData>> data
                            = WindwsService.DeviceMonitor.Program.LaohuaService.BatchSensorDataDictionary;
        ...
        public void Start() {
            ...
            bool b2 = base.MQ_Sub( new MqttMessage() { TopicName = "LaoHua/QueryRealtimeData" } ).Result;
        }
