...
namespace MKSS.APP.ECTester
{
    ...
    public partial class App : Application
    {
        ...
        protected override void OnStartup(StartupEventArgs e)
        {
           ...
           // ECTesterService.ECSensotMode = ECSensotMode.AcidityMode;//酸性两电极
            if (e.Args != null)
            {
                foreach (var item in e.Args)
                {
                    if (item.ToLower() == "AqueousMode".ToLower())
                    {
                        ECTesterService.ECSensotMode =  ECSensotMode.AqueousMode;//水性
                    }
                    if (item.ToLower() == "IndustryMode".ToLower())
                    {
                        ECTesterService.ECSensotMode = ECSensotMode.IndustryMode;//工业
                    }
                    if (item.ToLower() == "AcidityMode".ToLower())
                    {
                        ECTesterService.ECSensotMode = ECSensotMode.AcidityMode;//酸性两电极
                    }
                }
            ...
            SensorGroupData.ShowMode = ECTesterService.IndustryMode ? ShowModeEnum.AD : ShowModeEnum.DuanDianYa;

            ECTesterService.Debug = Appsettings.App(new string[] { "Debug" }).ObjToString().ToLower() == "true";
            ECTesterService.DebugPrint = Appsettings.App(new string[] { "DebugPrint" }).ObjToString().ToLower() == "true";
        ...
        void Log(string str)
        {
            System.IO.File.AppendAllLines(AppDomain.CurrentDomain.BaseDirectory + "\\log.log", new string[] { System.Environment.NewLine });
            System.IO.File.AppendAllLines(AppDomain.CurrentDomain.BaseDirectory + "\\log.log", new string[] { str });
        }
