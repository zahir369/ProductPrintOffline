            ...
            do
            {
                try
                {
                    lock (_syncLock)
                    {
                        ...
                        var dataframe = message.MessageFrame;
                       var datastr= BitConverter.ToString(dataframe);
                        Write(message);
                ...
                catch (Exception e)
                {
                    if (e is SocketException || e.InnerException is SocketException)
                    {
                        throw;
                             ...
                             e is IOException)
                    {
                        Logger.Error($"{e.GetType().Name}, {(_retries - attempt + 1)} retries remaining - {e.Message}");
