...
namespace MKSS.APP.UIBiaoDing
{
    public class UISheBeiBiaoDingPage
    {
        ...
        public override string ToString()
        {
            ...
            for (int i = From; i < To; i++)
            {
                if (i == From && Data[i].Address.V > 0) start = Data[i].Address.ToShortString();
                if (i <= To && Data[i].Address.V>0) end = Data[i].Address.ToShortString();
            }
            return string.Format("{0}", start, end);
        }
    }
