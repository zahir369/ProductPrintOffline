			...
			while (root.i < AddrList.Count)
			{
				...
				if (sucess)
				{
					...
					for (int i = 0; i < 15; i++)
					{
						...
						if (sno.ToString() == "2852126720" || sno.ToString() == "730144440320" || sno.ToString() == "0")//空位置
						{
					...
					if (this.DataEvent != null)
					{
                        ...
                        {
							this.MessageLog = "刷新【" + AddrList[line.Root.i] + "】到界面出错" + ex.Message;
							this.MessageLog = "刷新【" + AddrList[line.Root.i] + "】到界面出错" + ex.StackTrace;
                        }
					}
					await Task.Run(delegate ()
					{
