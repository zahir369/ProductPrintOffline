...
namespace MKSS.Service.LaoHua
{
    ...
    public class AllConnection : Dictionary<string, BoardConnection> {
                    ...
                    finally
                    {
                        ...
                        if (t >= totla)
                        {
                            ...
                            try
                            {
                                ...
                                Dictionary<string, bool> posDic = new Dictionary<string, bool>();
                                for (int i = 0; i < ProductModelGroupTable.Count; i++)
                                {
                                    ...
                                    {
                                        var rowData = ProductModelGroupTable[i].ProductTable[j];
                                        if (string.IsNullOrEmpty(rowData.Serial)) continue;
                                        if (!posDic.ContainsKey(rowData.Serial)) posDic.Add(rowData.Serial, true);
                                    }
                                }
                                //从服务器上获取不良品状态，刷新到界面
                                UISheBeiBiaoDingViewModel.Intance.SerialNoFactory.GetHEGE(posDic);
                                for (int i = 0; i < ProductModelGroupTable.Count; i++)
                                {
                                    SensorGroupDataModel mm = ProductModelGroupTable[i];
                                    if (mm.Address.V == 0) continue;
                                    for (int j = 0; j < mm.ProductTable.Count; j++)
                                    {
                                        var rowData = ProductModelGroupTable[i].ProductTable[j];
                                        if (string.IsNullOrEmpty(rowData.Serial)) continue;
                                        if (posDic.ContainsKey(rowData.Serial))
                                        {
                                            UISheBeiBiaoDingViewModel.Intance.ConnectionPool.Qualified[rowData.Address.APP, rowData.Address.V, j + 1] = posDic[rowData.Serial];
                                        }
                                    }
                                }
                            }
        ...
        internal void SetReadDeviceDateTime(DateTime DeviceDateTime, DateTime ManufacturingDate, int v)
        {
            ...
            foreach (var conn in this.Values.ToList())
            {
                Task taskSet = new Task(() =>
                {
                    ...
                    APP.UserControls.WaitWindow.ShowWindow("正在校时", "正在二次写入设备时间,出厂日期[" + DeviceDateTime + "][" + sucess1 + "-" + sucess2 + "-" + sucess3 + "/" + total + "]......", MKSS.APP.UIBiaoDing.UISheBeiBiaoDingViewModel.Intance.PageContext);
                    MKSS.APP.UIBiaoDing.Util.ViewUtil.Sleep(1000);
        ...
        internal void SetReadDeviceDateTime(SensorGroupDataModel groupData,DateTime DeviceDateTime, DateTime ManufacturingDate, int v)
        {
            int sucess1 = 0;
            int sucess2 = 0;
            int sucess3 = 0;
            var all = this.Values.Where(conn => conn.Address.Contains(groupData.Address)).ToList();
            int total = all.Count;
            APP.UserControls.WaitWindow.ShowWindow("正在校时", "正在写入设备时间,出厂日期[" + DeviceDateTime + "][" + sucess1 + "-" + sucess2 + "-" + sucess3 + "/" + total + "]......", MKSS.APP.UIBiaoDing.UISheBeiBiaoDingViewModel.Intance.PageContext);
            List<Task> allTask = new List<Task>();
            foreach (var conn in all.ToList())
            { 
                Task taskSet = new Task(() =>
                {
            ...
            foreach (var conn in this.Values.ToList())
            {
                Task.Run(delegate ()
                {
                    try
                    {
                        ...
                        foreach (var add in conn.Address)
                        {
                            conn.CommFactory.SetSpan(TxtSpan, 0); a1++;
            ...
            foreach (var conn in this.Values.ToList())
            {
                Task.Run(delegate ()
                {
                    try
                    {
                        ...
                        foreach (var add in conn.Address)
                        {
                            conn.CommFactory.SetZero(TxtZreo, 0); a1++;
