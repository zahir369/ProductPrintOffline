...
namespace MKSS.APP.SemiTester
{
	...
	public class UIZhuiSuData
    {
		...
		public static UIZhuiSuData Instance
        {
			get {
				if (_instance == null) {
					...
					_instance.RefreshData( new System.Diagnostics.Stopwatch());
		...
		public SensorGroupDataServices SensorDataServices { get; set; }

		public UIZhuiSuData() {
			BatchServices = new BatchServices(); 
			SensorServices = new SensorServices();
			SensorDataServices = new SensorGroupDataServices();
		}
		...
		public void RefreshData(Stopwatch watcher)
		{

			ULogger.Info("DBListBatch:" + watcher.Elapsed.TotalSeconds.ToString("f3"));
			DBListBatch = BatchServices.Query(wx => wx.F_AgingStatus == 1).Result;
			ULogger.Info("DBListBatchFinished:" + watcher.Elapsed.TotalSeconds.ToString("f3"));
			DBListBatchFinished = BatchServices.Query(wx => wx.F_AgingStatus != 1).Result;
			foreach (Batch item in DBListBatch)
			{

			}
			ULogger.Info("DBListBatchFinished finish:" + watcher.Elapsed.TotalSeconds.ToString("f3"));
		...
		public void PauseBatch(Batch _Batch, double F_AgingEndTime)
		{
			...
			try
			{
				...
				_Batch.F_AgingEndTimeActual = DateTime.Now;
				_Batch.F_AgingLastUpdateTime = DateTime.Now;
				_Batch.EnumAgingStatus = EnumAgingStatus.InAgingPaused;
				bool resccs = BatchServices.Update(_Batch).Result;

			}
			catch (Exception ex)
			{
				ULogger.Error(string.Format("结束暂停出错：{0}", ex.Message));
				ULogger.Error(ex);
			}

		}
		public void ResumeBatch(Batch _Batch, double F_AgingEndTime)
		{
			...
			{
				ULogger.Error(string.Format("结束继续出错：{0}", ex.Message));
				ULogger.Error(ex);
			}
		...
		public void DeleteBatch(Batch batch)
		{
			...
			try
			{
				...
				string create = @"
					DROP TABLE IF EXISTS pd_sensorgroupdata_{0};
		...
		public long NextIdBatch
		{
			get
			{
				long year = long.Parse(DateTime.Now.ToString("yyyyMMdd"));
				long year_from = year * 1000 + 0;
				long year_to = (year + 1) * 1000 + 1;
