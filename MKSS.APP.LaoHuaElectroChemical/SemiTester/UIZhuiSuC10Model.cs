...
namespace MKSS.APP.SemiTester
{
    ...
    public class UIZhuiSuC10Model {
        ...
        public LaoHuaDataProvider DataProvider = LaoHuaDataProvider.Instance;
        ...
        public void ECService_OnFinish(Batch _Batch, TimeSpan F_AddTime)
        {
        ...
        public void SemiTesterService_OnNewData(Batch _Batch, SensorGroupData datas_history, TimeSpan F_AddTime)
        {
        ...
        public void RefrehPageInner(Model.Batch _Batch, SensorGroupData datas_history, TimeSpan F_AddTime,UIZhuiSuC10 con10) {
            Stopwatch watcher = new Stopwatch();
            watcher.Start();
            if (UIChartWindows.Instance != null) {
                UIChartWindows.Instance.AppendPoint(_Batch,datas_history);
            }
        ...
        public void IniPageData(Batch batch)
        {
            if (batch == null) {
                return;
                //batch = Data.BatchServices.BaseDal.Db.Queryable<Batch>()
                //    .OrderBy(w => w.F_BatchId, SqlSugar.OrderByType.Desc).First();
            }
            BatchCurrent = batch;
            this.View.UIZhuiSuC10SensorsPage.UIZhuiSuC10SensorsSET.IniData(batch, new Stopwatch());
            DataProvider.Query(batch);
            DataProvider.Query(batch, MqttTaskCommand.StatusTask);
            DataProvider.Query(batch, MqttTaskCommand.QueryRealtimeData);
        }
    ...
    public class ElectroChemicalMqttClient  { 
