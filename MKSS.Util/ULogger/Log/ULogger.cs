...
namespace MKSS.Util.Log
{
    ...
    public class ULogger
    {
        ...
        public static string LogDirecrory { get; set; } = "App_log";
        ...
        {
            string dir = MKSS.Util.Log.ULogger.StartupWebRoot + "\\"+ LogDirecrory + "\\" ;
        ...
        public static string CurrentLogFile(string title) {
            string dir = MKSS.Util.Log.ULogger.StartupWebRoot + "\\" + LogDirecrory + "\\" + DateTime.Now.ToString("yyyyMMdd") + "\\";
            if (!System.IO.Directory.Exists(dir))
            {
                Directory.CreateDirectory(dir); 
                //if (LoggerEf != null)
        ...
        public static void Info(object stringValue, ULogDataFilter filter = null, bool newLine = true)
        { 
            Log(stringValue + "",   filter = null);
        }

        public static void Log(string str, ULogDataFilter filter = null, bool newLine = true)
        {
            try
            {
                ...
                LogInner( showlog,  class_current,  str,   filter,  newLine  );
        ...
        public static void LogBy(string str,Type current, LogLevel logLevel, ULogDataFilter filter = null, bool newLine = true)
        {
            try
            {
        ...
        public static void LogBy(string str, string current, LogLevel logLevel, ULogDataFilter filter = null, bool newLine = true)
        {
            try
            {
        ...
        static void LogInner(bool showlog, LogTagClass class_current,string str, ULogDataFilter filter = null,bool newLine = true) {
            ...
            try
            {
                ...
                if (!LogToFileIgnore)
                {
                    ...
                    if (log_to_file)
                    {
                        ...
                        if (newLine)
                        {
                            File.AppendAllText(filename, DateTime.Now.ToString("HH:mm:ss.fff") + "【" + System.Threading.Thread.CurrentThread.ManagedThreadId + "】:" + str + System.Environment.NewLine);
                        }
                        else
                        {
                        ...
                        string dir_yesterday = MKSS.Util.Log.ULogger.StartupWebRoot + "\\" + LogDirecrory + "\\" + yesterday.ToString("yyyyMMdd") + "";
            ...
            if (!AuthroizChecked)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        string msg1 = "";
                        //临时授权未通过，正式有授权
                        Authroized = ComputerInfo.Instance.Match(new SHA512CryptoServiceProvider(), StartupPath + "\\MKSS.Util.License.lic",ref msg1);
                        if (Authroized)
                        {
                            return;
                        }
                        if (OnErrorLicense != null) OnErrorLicense(pathSrc, msg1);
                        return;
                    }
                    catch { }
                ...
                if (AuthroizChecked && !Authroized)
                {
                    ...
                    try
                    {
                        ...
                        if (OnErrorLicense != null) OnErrorLicense(pathSrc, msg);
        ...
        public static void Info(int i,string p, ULogDataFilter filter = null, bool newLine = true)
        {
            Log(p,filter,   newLine  );
        }

        public static void Info(string p, ULogDataFilter filter = null, bool newLine = true)
        {
            Log(p,filter,   newLine );
        }
    ...
    public delegate void ULogOnErrorLicense(string srcPath,string msg);
