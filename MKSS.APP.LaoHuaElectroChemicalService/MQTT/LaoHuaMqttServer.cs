...
namespace MKSS.APP.LaoHuaService.MQTT
{
    ...
    public class LaoHuaMqttServer 
    {
        static LaoHuaMqttServer _Instance = null;
        public static LaoHuaMqttServer Instance
        {
            get {
                if (_Instance == null) {
                    _Instance = new LaoHuaMqttServer();
                }
                return _Instance;
            }
        }
        ...
        public async void StartMqttServerAssert()
        {
            try
            {
                if (mqttServer == null)
                {
                    LaoHuaMqttServerConfig LaoHuaMqttServrConfig = LaoHuaMqttServerConfig.Instance;


                    optionsBuilder = new MqttServerOptionsBuilder()
                    .WithDefaultEndpoint().WithDefaultEndpointPort(LaoHuaMqttServrConfig.ServerPort).WithConnectionValidator(
                        c =>
                        {
                            var currentUser = LaoHuaMqttServrConfig.ServerLoginName;
                            var currentPWD = LaoHuaMqttServrConfig.ServerLoginPwd;
