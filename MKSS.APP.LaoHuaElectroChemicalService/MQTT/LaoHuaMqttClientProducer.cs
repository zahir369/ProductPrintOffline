...
using MKSS.Model;
using MKSS.APP.LaoHuaElectroChemicalService;
...
namespace MKSS.APP.LaoHuaService.MQTT
{
    ...
    public class LaoHuaMqttClientProducer : LaoHuaMqttClient {
        ...
        public static LaoHuaMqttClientProducer Instance
        {
            get
            {
                if (_Instance == null)
                {
                    ...
                    ElectroChemicalService.OnNewData += _Instance.LaohuaService_OnNewData;
        ...
        private void ElectroChemicalService_OnSucess(Batch _Batch, int floor, string region)
        {
            ...
            mes.TopicName = "LaoHua/OnSucessResponse/" + _Batch.F_BatchId;
        ...
        private void ElectroChemicalService_OnError(Batch _Batch, int floor, string region, string message)
        {
            ...
            MqttMessage mes = new MqttMessage();
            mes.TopicName = "LaoHua/OnErrorResponse/" + _Batch.F_BatchId;
            mes.PayLoad = JsonConvert.SerializeObject(status);
            MqttClientPublishResult bxx = base.MQ_PubWaiting(mes);
        ...
        {
            Task t = new Task(()=>{
                string PayLoad = mqMessage.PayLoad;
                string[] PayLoadArr = PayLoad.Split(";");
                long batch_id_int = 0;
                Batch _Batch = null;
                if (!long.TryParse(PayLoadArr[0], out batch_id_int)) {
                ...
                if (mqMessage.TopicName.StartsWith("LaoHua/"+ MqttTaskCommand.QueryRealtimeData)) {
                    ...
                    if (mqMessage.TopicName.StartsWith("LaoHua/" + MqttTaskCommand.SetHidden))
                    {
                        //ULogger.Info(string.Format("任务继续命令:{0}", mqMessage.TopicName));
                        _Batch = ElectroChemicalService.OfBatch(batch_id_int);
                        Program.LaohuaService.SetVisible(int.Parse(PayLoadArr[1]), false);
                    }
                    if (mqMessage.TopicName.StartsWith("LaoHua/" + MqttTaskCommand.SetVisible))
                    {
                        //ULogger.Info(string.Format("任务继续命令:{0}", mqMessage.TopicName));
                        _Batch = ElectroChemicalService.OfBatch(batch_id_int);
                        Program.LaohuaService.SetVisible(int.Parse(PayLoadArr[1]), true);
                    }
        ...
        public void PublishStatus(Batch _Batch)
        {
            ...
            StatusEntity status = new StatusEntity()
            {
                ...
                FloorsVisible = Program.LaohuaService.Connection.FloorsVisible,
            };
            MqttMessage mes = new MqttMessage();
