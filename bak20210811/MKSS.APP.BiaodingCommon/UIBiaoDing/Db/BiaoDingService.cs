...
namespace MKSS.Service.UIBiaoDing
{
    ...
    public class BiaoDingService
	{
		...
		/// <returns></returns>
		public MKSS.Model.SensorData this[int boardNo, int slotNo] {
			get {
		...
		public void StartSensorData(List<DeviceDataMonitorWPF.UIBiaoDing.Util.Address> addrs) {
            ...
            try
            {
				...
				foreach (DeviceDataMonitorWPF.UIBiaoDing.Util.Address addr in addrs)
				{
					SensorGroupDataModel mdata = UISheBeiBiaoDingViewModel.Intance.GroupDataOfAddress(addr);
					for (int s = 1; s <= 15; s++)
					{
						string sensorId = Sensor.CreateCensorId(batch.F_BatchId, addr.V, s);
		...
		public void PushData(SensorGroupData pm, ReadMode mode)
		{
			...
			foreach (DeviceDataMonitorWPF.UIBiaoDing.Util.SensorDataX item in pm.SingleAddressData)
			{
				string F_SensorId = Sensor.CreateCensorId(CurrentBatch.F_BatchId, pm.AddressInt, item.Position);
				switch (mode)
				{
					case ReadMode.RX01:
		...
		public void SetHEGE(Address addr,int Position, bool hege) {
			...
			string F_SensorId = Sensor.CreateCensorId(CurrentBatch.F_BatchId, addr.V, Position);
			...
			else
			{
				...
				config.StaTotal = UISheBeiBiaoDingViewModel.Intance.SerialModBus.Qualified.StaTotal;
				config.StaUnqualified = UISheBeiBiaoDingViewModel.Intance.SerialModBus.Qualified.StaUnqualified;
				config.StaQuallified = UISheBeiBiaoDingViewModel.Intance.SerialModBus.Qualified.StaQuallified;
				config.StaRate = UISheBeiBiaoDingViewModel.Intance.SerialModBus.Qualified.StaRate == double.NaN ? "" : UISheBeiBiaoDingViewModel.Intance.SerialModBus.Qualified.StaRate.ToString("P1");
