...
namespace DeviceDataMonitorWPF.UIBiaoDing
{
	...
	;
		...
		public async Task BtnSetZero()
		{
			...
			{
				foreach (var conn in this.ConnectionPool.Values.ToList())  conn.CommFactory.SetZero(TxtZreo);
			});
		}
		...
		public async Task BtnSetSpan()
		{
			...
			await Task.Run(delegate ()
			{
				foreach (var conn in this.ConnectionPool.Values.ToList())  conn.CommFactory.SetSpan(TxtSpan);
			});
		}
   ...
   //         }
		...
		public async Task BtnQuery()
		{
			...
			await this.ConnectionPool.ReadAll(); 
		...
		private void SM_DataEvent(object sender, CommFactory.DataEventArgs e)
		{
				...
				{ 
					...
					switch (e.ReadMode)
					{
						...
						case ReadMode.RX7D_ReadAutoAdjustStatus:
							...
							break;
						case ReadMode.RX95_ReadSerialNo:
							for (int l = 0; l < 15; l++)
							{
								DataList[l].Serial = e.ProductData.SingleAddressData[l].Serial;
							}
							break;
						case ReadMode.RXB2_ReadDeviceDateTime:
							for (int l = 0; l < 15; l++)
							{
			...
			catch (System.Exception ex)
			{
				...
				/// 最快半秒刷新一次，避免界面刷新太多卡了
				if (DateTime.Now - SM_DataEventLastUIRefresh > new TimeSpan(0, 0, 0, 0, 800)) {
