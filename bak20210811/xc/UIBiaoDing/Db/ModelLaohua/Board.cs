...
namespace MKSS.Model.Laohua
{
	...
	public class Board
	{
        ...
        [SugarColumn(IsIgnore = true)]
        public List<byte> AddrList { 
            get {
                List<byte> ret = new List<byte>();
                if (string.IsNullOrEmpty(F_BoardAddress)) return ret;
                string[] arr = F_BoardAddress.Split(',');
                foreach (var item in arr)
                {
                    int i = 0;
                    if (int.TryParse(item, out i))
                    {
                        if (i <= 255 && i >= 0) ret.Add((byte)i);
                    }
                }
                return ret; 
            } 
        ...
        [SugarColumn(IsIgnore = true)]
        public List<long> DataBusList
        {
            get
            {
                List<long> ret = new List<long>();
                if (string.IsNullOrEmpty(F_DataBusId)) return ret;
                string[] arr = F_DataBusId.Split(',');
                foreach (var item in arr)
                {
                    long i = 0;
