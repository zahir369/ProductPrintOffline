...
namespace MKSS.Service.LaoHua
{
	...
	public class BoardConnectionSreial : BoardConnection
	{
		...
		/// 是否已经打开
		/// </summary>
		public override bool IsOpen { get { return this._serial != null && this._serial.IsOpen; } }
		/// <summary>
		///  检查连接 
		/// </summary>
		/// <returns></returns>
		...
		{ 
			...
			if (isOpen)
			{
				CommFactory.Message = "error:串口设备已被其它端口占用";
			}
			else
			{
				bool flag = COM == "";
				...
				else
				{
					...
					CommFactory.Master = modbusFactory.CreateRtuMaster(this._serial);
