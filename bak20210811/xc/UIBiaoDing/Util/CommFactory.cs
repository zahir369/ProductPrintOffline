...
namespace DeviceDataMonitorWPF.UIBiaoDing.Util
{
    ...
    public class CommFactory
	{
		...
		private ReadMode _ReadMode = ReadMode.WX01_StartRead;
		...
		private void ReadSignal(DeviceDataMonitorWPF.UIBiaoDing.Signal.ISignal signal)
		{
			...
			try
			{
				this.Master.WriteMultipleRegisters(0, 0, signal.GetSignalBytes());
			}
			catch
			{
			}
		...
		private bool WriteSignal(DeviceDataMonitorWPF.UIBiaoDing.Signal.ISignal signal,int times =1,byte addr = 0,bool clean_databus = true)
		{
			...
			if (clean_databus) {
				...
				try
				{
					this.Master.WriteMultipleRegisters(addr, 0, StopReadSignal.GetSignalBytes()); 
				}
            ...
            for (int i = 0; i < times; i++)
			{
				...
				try
				{
					this.Master.WriteMultipleRegisters(addr, 0, signal.GetSignalBytes());
				}
		...
		public void SetZero(int Zero, int TemperatureZero, int DifferenceValue)
		{
			...
			this.IsWrite = false;
			this.Message = (sucess ? string.Format("已发送校准传感器零点({0})指令；", Zero) : "标定失败");
		}
		...
		public void SetZero(int Zero)
		{
			...
			this.IsWrite = false;
			this.Message = string.Format("已发送校准传感器零点({0})指令；", Zero);
		}
		...
		public void SetSpan(int Span)
		{
			...
			this.IsWrite = false;
			this.Message = string.Format("已发送" + this.Connection + "校准传感器Span({0})指令；", Span);
		}
		...
		public async Task ReadLiangCheng(List<Address> AddrList)
		{
			...
			while (root.i < AddrList.Count)
			{
				...
				await Task.Run(delegate ()
				{
					...
					{
						lineExt.LineData.Root.XTHIS.Message = "【" + AddrList[line.Root.i] + "】" + ex.ToString();
					}
				});
				lineExt = null;
		...
		public async Task ReadSerialNo(List<Address> AddrList)
		{
			...
			await Task.Run(delegate ()
			{
				this._ReadMode = ReadMode.RX95_ReadSerialNo;
			...
			while (root.i < AddrList.Count)
			{
				...
				await Task.Run(delegate ()
				{
					...
					{
						lineExt.LineData.Root.XTHIS.Index = lineExt.LineData.Root.i;
						Thread.Sleep(Config.BiaoDingConfgig.Instance.ReadSerialNoWait1);//等待足够时间，让总线把数据汇总完
						lineExt.tempH = lineExt.LineData.Root.XTHIS.Master.ReadHoldingRegisters(AddrList[lineExt.LineData.Root.i].Vb, 0, 75);
						Thread.Sleep(Config.BiaoDingConfgig.Instance.ReadSerialNoWait2);//等待足够时间，让总线把数据汇总完
		...
		public async Task ReadDeviceDateTime(List<Address> AddrList)
		{
			...
			await Task.Run(delegate ()
			{
				this._ReadMode = ReadMode.RXB2_ReadDeviceDateTime;
		...
		{
			if (!Connected) return;
			this._ReadMode = ReadMode.WX01_StartRead;
			...
			while (this.IsRead)
			{
				...
				while (root.i < AddrList.Count)
				{
					...
					{
						line.Root.XTHIS.Message = "【" + AddrList[line.Root.i] + "】" + ex.ToString();
					}
			...
			while (this.IsRead)
			{
				...
				while (root.i < AddrList.Count)
				{
					...
					Task<bool> sendEmailMessage = Task.Run(() => {
						...
						{
							line.Root.XTHIS.Message = "【" + AddrList[line.Root.i] + "】" + ex.Message;
							return false;
						}
					...
					if (result) {
                        ...
                        {
							Message = string.Format("EX：{0},{1}===》{2},{3}", this.Connection.ToString(), addrs, AddrList[line.Root.i].Vb, exx.Message);
						}
