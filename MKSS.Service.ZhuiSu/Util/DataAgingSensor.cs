...
namespace MKSS.Service.ZhuiSu.Util
{
    ...
    public class DataAgingSensor : INotifyPropertyChanged
	{
		...
		public double? ValueFormate { get { return Value == null ? null :  (double)Value.Value / (double)10000 * 5; } }
		...
		public void PashData(int val, DateTime now) {
			Now = now;
			Value = val;
			DataAgingSensorValue v = new DataAgingSensorValue() { Time = Now, Value = val };
			Values.Add(v);
			foreach (DataAgingRange item in Ranges.Values)
			{
				item.Refresh(v); 
        ...
        {
            return string.Format("{0}_{1}:{2},{3}", Address, Position, Value, ValueFormate);
        }
