...
namespace MKSS.APP.LaoHua.UILaoHua
{
    ...
    public class UILaoHuaGuanChaAddModel { 
		...
		public string TxtSerialPrefix { get; set; }
		public double TxtTimeTotal { get; set; } = 72;
		...
		{
			var bbsids = bbs.Select(w => w.F_BoardId).ToList();
			foreach (Board item in this.SelectBatchBoard.Keys.ToList())
			{
				foreach (BoardCase casee in Data.DBListBoardCase)
				{
					...
					{
						if (bbsids.Contains(item.F_BoardId))
						{
		...
		public void AddBatch(List<SensorInfo> SensorInfos)
		{
			...
			try
			{
				...
				batch.F_SCRWD_Date = string.IsNullOrEmpty(SelectRuleEntity.Date) ? DateTime.MinValue: DateTime.Parse(SelectRuleEntity.Date);
				batch.F_SCRWD_Department = (SelectRuleEntity.Department);
				batch.F_SCRWD_OrderID = (SelectRuleEntity.OrderID).ToString();
				batch.F_SCRWD_OrderNumber = (SelectRuleEntity.OrderNumber);
				batch.F_SCRWD_ProductCode = (SelectRuleEntity.ProductCode);
				batch.F_SCRWD_ProductFullName = (SelectRuleEntity.ProductFullName);
				batch.F_SCRWD_ProductType = (SelectRuleEntity.ProductType);
				batch.F_SCRWD_Qty = (SelectRuleEntity.Qty);
					...
					foreach (Board b in Data.DBBoardCaseDictionary[item])
					{
						...
						if (v)
						{
							...
							for (int i = 0; i < b.F_SensorCount; i++)
							{
								...
								if (SensorInfos != null) {
									SensorInfo info = SensorInfos.FirstOrDefault(w => w.F_BoardCaseId == sen.F_BoardCaseId && w.F_BoardId == sen.F_BoardId && w.F_SlotNO == sen.F_SlotNO);
									if (info != null) {
										sen.F_SerialNO = info.F_SerialNO;
										sen.F_DeviceHistoryClock = info.F_DeviceHistoryClock;
										sen.F_DeviceHistoryClockDetectTime = info.F_DeviceHistoryClockDetectTime;
										sen.F_SerialNoDetectTime = info.F_SerialNoDetectTime;
									}
								}
		...
		public SerialNoRuleEntity SelectRuleEntity { get { return UISheBeiBiaoDingViewModel.Intance.SelectRuleEntity; } }
		public async Task BtnScrwd()
		{
			Window ww = UISheBeiBiaoDingViewModel.Intance.SerialNoFactory.SerialNoWindow();
			...
			{
				this.TxtProjectTitle = string.Format("{0}[{1}]", SelectRuleEntity.ProductFullName, SelectRuleEntity.OrderNumber);
				this.TxtProductName = SelectRuleEntity.ProductFullName;
				this.TxtProductCode = SelectRuleEntity.ProductCode;
				this.TxtSerialPrefix = SelectRuleEntity.OrderNumberPrefix;
				this.PageContext.TxtProductName.Text = this.TxtProductName;
				this.PageContext.TxtSerialPrefix.Text = this.TxtSerialPrefix;
