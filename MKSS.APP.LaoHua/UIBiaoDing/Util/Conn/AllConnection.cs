...
namespace MKSS.Service.LaoHua
{
    ...
    public class AllConnection : Dictionary<string, BoardConnection> {
        ...
        {
            APP.UserControls.WaitWindow.ShowWindow("正在校时", "正在写入出厂时间[" + dateTime + "]......", MKSS.APP.UIBiaoDing.UISheBeiBiaoDingViewModel.Intance.PageContext);
            foreach (var conn in this.Values.ToList())
            {
        ...
        {
            APP.UserControls.WaitWindow.ShowWindow("正在校时", "正在写入设备时间[" + now + "]......", MKSS.APP.UIBiaoDing.UISheBeiBiaoDingViewModel.Intance.PageContext);
            foreach (var conn in this.Values.ToList())
            {
                Task parent = new Task(() =>
                {
