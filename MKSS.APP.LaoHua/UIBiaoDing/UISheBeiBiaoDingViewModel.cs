...
namespace MKSS.APP.UIBiaoDing
{
    public class UISheBeiBiaoDingViewModel : Screen 
	{
		...
		public bool CanBtnReadSerialNo { get { return ConnIsOpen && AddressAvaliable; } }
		public bool CanBtnReadTime { get { return ConnIsOpen && AddressAvaliable; } } 
		...
		public MKSS.APP.LaoHua.MainWindow MainView { get; set; }
		public UISheBeiBiaoDingViewModel()
		{
            ...
            {
				MessageBox.Show(ex.Message, "UISheBeiBiaoDingViewModel出错");  
            }
	...
	;
		...
		public void BtnConnect()
        {
			...
			if (TxtPortsModeSelect == (CONN_TYPE_BOARDCASE))
			{
				...
				List<Board> BoardList = this.BoardCaseSelectEntity.SelectBoard.Where(w => w.Value).Select(w => w.Key).ToList();
                ...
                foreach (var item in DataBusList.Values)
                {
					...
					this.ConnectionPool.Push(new BoardConnectionBoardCase(
						 item.BoardCase, item.DataBus, item.Address, this.ConnectionPool, (BiaoDingConfgig.Instance.RefreshInterval * 4)));
				}
		...
		public async Task BtnStartBatch()
		{
			List<SensorInfo> sensconfig = new List<SensorInfo>();
			List<Board> BoardList = new List<Board>();
			//只识别老化柜模式的数据
			if (this.TxtPortsModeSelect == CONN_TYPE_BOARDCASE) {
				BoardList = this.BoardCaseSelectEntity.SelectBoard.Where(w => w.Value).Select(w => w.Key).ToList();
				for (int i = 0; i < this.ProductModelGroupTable.Count; i++)
				{
					SensorGroupDataModel mm = ProductModelGroupTable[i];
					...
					for (int j = 0; j < mm.ProductTable.Count; j++)
					{
						SensorGroupDataModel g = this.ProductModelGroupTable[i];
						SensorDataX x = this.ProductModelGroupTable[i].ProductTable[j];
						Address addr = x.Address;
						SensorInfo info = new SensorInfo()
						{
							F_SlotNO = addr.V % 2 == 0 ? 15 + x.Position : x.Position,
							F_SerialNO = x.Serial,
							F_DeviceHistoryClock = x.DeviceDateTime,
							F_SerialNoDetectTime = DateTime.Now,
							F_DeviceHistoryClockDetectTime = DateTime.Now
						};

						BoardCase _BoardCase = this.BoardCaseSelectEntity.DBListBoardCase.FirstOrDefault(w => w.F_BoardCaseAddress == addr.APP );
						if (_BoardCase != null) {
							info.F_BoardCaseId = _BoardCase.F_BoardCaseId;
							Board _Board = this.BoardCaseSelectEntity.DBBoardCaseDictionary[_BoardCase].FirstOrDefault(w => w.AddrList.Contains(addr.Vb));
							if (_Board != null) {
								info.F_BoardId = _Board.F_BoardId;
							}
						}
			...
			LaoHua.UILaoHua.UILaoHuaGuanChaAdd w = new LaoHua.UILaoHua.UILaoHuaGuanChaAdd(BoardList, UISheBeiBiaoDingViewModel.Intance.SelectRuleEntity, sensconfig);
			w.Topmost = true;
            ...
            LaoHua.UILaoHuaGuanChaData.Instance.RefreshData();
		}
				...
				if (dlg.ShowDialog() == true)
				{
					 
					{
						try
						{
							...
							ETE.SetCellValue<DateTime>(hr, 2, DateTime.Now, "yyyy-MM-dd HH:mm", hr, 3);
		...
		public async Task BtnReadTime()
		{
			...
			WaitWindow.ShowWindow("正在读取", "正在读取设备时间......", PageContext);
	...
	public class SensorInfo
	{
		...
		/// 串号读取时间
		/// </summary>		
		public DateTime F_SerialNoDetectTime { get; set; }
		/// <summary>
		/// 设备时间
		/// </summary>		
		public string F_DeviceHistoryClock { get; set; }
		/// <summary>
		/// 设备时间读取时间
		/// </summary>		
		public DateTime F_DeviceHistoryClockDetectTime { get; set; }
