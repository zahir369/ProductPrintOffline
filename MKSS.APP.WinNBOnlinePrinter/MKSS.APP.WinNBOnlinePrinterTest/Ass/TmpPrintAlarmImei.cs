...
{
    public class TmpPrintAlarmImei : TmpPrintAbs
    {

        pd_device D1;
        pd_device D2;
        public override string TmpName { get; set; } = "防盗报警器IMEI.btw";
        ...
        public override bool PrintBar(bool isPrint = false)
        {

            {
                ...
                try
                {
                    ...
                    {
                        labelFormat.SubStrings.SetSubString("QRCODE", dev1.F_IMEI);
                    }
                    else
                    {
                        labelFormat.SubStrings.SetSubString("QRCODE", "000000000000000");
                      
                    }

                    if (dev2 != null && !string.IsNullOrEmpty(dev2.F_IMEI))
                    {
                        labelFormat.SubStrings.SetSubString("QRCODE", dev2.F_IMEI);
                    }
                    else
                    {
                        labelFormat.SubStrings.SetSubString("QRCODE", "000000000000000");
