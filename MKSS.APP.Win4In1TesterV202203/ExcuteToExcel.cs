...
namespace MKSS.APP.UIBiaoDing.Util
{
    public class ExcuteToExcel : IDisposable
    {
        ...
        public ExcuteToExcel(FileInfo filePath,string sheetname)
        {
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            this.Package = new ExcelPackage(filePath);
            CreateSheet(sheetname);
        }
    ...
    public class ChartHelper
    {
        ...
        public static void AddSeries(Chart chart, string seriesName, SeriesChartType chartType, Color color, Color markColor, ChartValueType xValueType, ChartValueType yValueType, bool showValue = false)
        {
            chart.Series.Add(seriesName);
            chart.Series[seriesName].ChartType = chartType;
            chart.Series[seriesName].Color = color;
            chart.Series[seriesName].XValueType = xValueType;
            chart.Series[seriesName].YValueType = yValueType;
            //设置显示的时间类型。
            if (showValue)
            {
