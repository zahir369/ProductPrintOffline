...
namespace MKSS.Service.LaoHua.Util
{
    ...
    public class ModBusBoardLogger : ModbusLogger
	{
        ...
        ModBusBoardConnection Connection;

        public ModBusBoardLogger(ModBusBoardConnection busConfig,LoggingLevel minimumLoggingLevel = LoggingLevel.Debug)
		...
		protected override void LogCore(LoggingLevel level, string message)
		{
            ...
            try
            {
                lock (this.locked) {
                    ...
                    string dir = MKSS.Util.Log.ULogger.StartupWebRoot + "\\" + MKSS.Util.Log.ULogger.LogDirecrory + "\\" + DateTime.Now.ToString("yyyyMMdd") + "\\";
                    ...
                    sw.Write(DateTime.Now.ToString("HH:mm:ss.fff") + "["+ Connection.ToString() + "]:" + message + System.Environment.NewLine);
                    sw.Close();
                    sw.Dispose();
                    fileStream.Close();
                    fileStream.Dispose();
