...
namespace MKSS.Service.LaoHua
{
	...
	public class BoardConnection : ULogDataFilter
	{
		...
		public ElectroChemicalService CommFactory { get; private set; }
		...
		public BoardConnection( int read_speed) {
			this.CommFactory = new ElectroChemicalService(this,   read_speed);
			FloorsVisible = new Dictionary<int, bool>();
            for (int i = 1; i <= 15; i++)
            {
				FloorsVisible.Add(i,true);
			}
			modbusFactory = new ModbusFactory(new List<IModbusFunctionService>(), true, new CommLogger(LoggingLevel.Trace,this));
		}
